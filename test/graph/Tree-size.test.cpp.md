...
data:
  ...
  bundledCode: "#line 1 \"test/graph/Tree-size.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/872\"\
    ...
    \ += e.second*tree.size[e.first]*(N-tree.size[e.first])*2LL;\n\tcout << ans <<\
    \ endl;\n\treturn 0;\n}\n"
  code: "#define PROBLEM \"https://yukicoder.me/problems/no/872\"\n\n#include <vector>\n\
    ...
    \ tree(N);\n\tfor(int i = 0; i < N-1; ++i){\n\t\tint u,v,w;\n\t\tcin >> u >> v\
    \ >> w;\n\t\tu--,v--;\n\t\ttree.make_edge(u,v,w);\n\t\ttree.make_edge(v,u,w);\n\
    \t}\n\ttree.make_size();\n\tlong long ans = 0;\n\tfor(int pa:tree.order) for(auto\
    \ e:tree.child[pa]) ans += e.second*tree.size[e.first]*(N-tree.size[e.first])*2LL;\n\
    \tcout << ans << endl;\n\treturn 0;\n}"
  dependsOn:
  - lib/graph/Tree.cpp
