...
data:
  ...
  bundledCode: "#line 1 \"test/graph/Tree-rerooting.test.cpp\"\n#define PROBLEM \"\
    ...
    \ 14 \"test/graph/Tree-rerooting.test.cpp\"\n\nint main(void){\n    int N,M,Q;\
    \ cin >> N >> M >> Q;\n    UnionFindTree uf(N);\n    Graph<long long> g(N);\n\
    \    for(int i=0;i<M;++i) {\n        int u,v; cin >> u >> v;\n        u--,v--;\n\
    \t\tuf.merge(u,v);\n        g.make_bidirectional_edge(u,v,1);\n    }\n    auto\
    \ tree = StaticTree<StaticTreeOperator<long long>>::builder(g).root().parent().ancestor().child().build();\n\
  ...
  requiredBy: []
  timestamp: '2023-06-17 04:29:45+09:00'
