...
# :heavy_check_mark: test/geometory/ConvexHullTrickMonotone-min.test.cpp
...
{% raw %}
```cpp
#line 1 "test/geometory/ConvexHullTrickMonotone-min.test.cpp"
#define PROBLEM "https://yukicoder.me/problems/no/409"
...
public:
	...
	void insert(TypeValue a, TypeValue b) {
		...
		//不必要な直線を取り除く
		while (lines.size() > 1 && is_not_required(lines[lines.size()-2], lines[lines.size()-1], {a,b})) lines.pop_back();
		lines.push_back({a,b});
	}
	...
	TypeValue get_monotone(TypeValue x) {
...
//傾きがa1<=a2<=a3...となるようにinsertする
#line 8 "test/geometory/ConvexHullTrickMonotone-min.test.cpp"
using ll = long long;
