...
data:
  ...
  bundledCode: "#line 1 \"old/Segment_Tree_Range_Gcd_Query.cpp\"\n#include <bits/stdc++.h>\n\
    ...
    \tsize_t N,M;\n\tT ini;\n\tvector<T> node;\n\npublic:\n\tSegment_Tree_Range_Gcd_Query(const\
    \ vector<T> & ar, const T ini) : M(ar.size()), ini(ini) {\n\t\tfor (N = 1; N <\
    \ M; N *= 2);\n\t\tnode.resize(2 * N - 1, ini);\n\t\tfor (int i = 0; i<M; ++i)\
    \ node[i + N - 1] = ar[i];\n\t\tfor (int i = N - 2; i >= 0; --i) node[i] = Gcd(node[2\
    \ * i + 1], node[2 * i + 2]);\n\t}\n\n\tSegment_Tree_Range_Gcd_Query(const size_t\
  ...
  code: "#include <bits/stdc++.h>\nusing namespace std;\n\ntemplate<class T> class\
    \ Segment_Tree_Range_Gcd_Query {\n\tsize_t N,M;\n\tT ini;\n\tvector<T> node;\n\
    \npublic:\n\tSegment_Tree_Range_Gcd_Query(const vector<T> & ar, const T ini) :\
    \ M(ar.size()), ini(ini) {\n\t\tfor (N = 1; N < M; N *= 2);\n\t\tnode.resize(2\
  ...
  verifiedWith: []
documentation_of: old/Segment_Tree_Range_Gcd_Query.cpp
layout: document
redirect_from:
- /library/old/Segment_Tree_Range_Gcd_Query.cpp
- /library/old/Segment_Tree_Range_Gcd_Query.cpp.html
