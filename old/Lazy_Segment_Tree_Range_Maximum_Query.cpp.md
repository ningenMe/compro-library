...
data:
  ...
  bundledCode: "#line 1 \"old/Lazy_Segment_Tree_Range_Maximum_Query.cpp\"\n#include\
    ...
    \npublic:\n\tLazy_Segment_Tree_Range_Maximum_Query(const vector<T>& ar, const\
    \ T ini) : M(ar.size()), ini(ini) {\n\t\tfor (N = 1; N < M; N *= 2);\n\t\tnode.resize(2\
    \ * N - 1);\n\t\tlazy.resize(2 * N - 1, ini);\n\t\tlazyflag.resize(2 * N - 1,\
    \ false);\n\t\tfor (int i = 0; i<M; ++i) node[i + N - 1] = ar[i];\n\t\tfor (int\
    \ i = N - 2; i >= 0; --i) node[i] = max(node[i * 2 + 1], node[i * 2 + 2]);\n\t\
    }\n\n\tLazy_Segment_Tree_Range_Maximum_Query(const size_t M, const T ini) : M(M),\
  ...
  code: "#include <bits/stdc++.h>\nusing namespace std;\n\ntemplate<class T> class\
    \ Lazy_Segment_Tree_Range_Maximum_Query {\n\tsize_t N, M;\n\tT ini;\n\tvector<T>\
    \ node, lazy;\n\tvector<bool> lazyflag;\n\npublic:\n\tLazy_Segment_Tree_Range_Maximum_Query(const\
    \ vector<T>& ar, const T ini) : M(ar.size()), ini(ini) {\n\t\tfor (N = 1; N <\
    \ M; N *= 2);\n\t\tnode.resize(2 * N - 1);\n\t\tlazy.resize(2 * N - 1, ini);\n\
    \t\tlazyflag.resize(2 * N - 1, false);\n\t\tfor (int i = 0; i<M; ++i) node[i +\
    \ N - 1] = ar[i];\n\t\tfor (int i = N - 2; i >= 0; --i) node[i] = max(node[i *\
    \ 2 + 1], node[i * 2 + 2]);\n\t}\n\n\tLazy_Segment_Tree_Range_Maximum_Query(const\
  ...
  verifiedWith: []
documentation_of: old/Lazy_Segment_Tree_Range_Maximum_Query.cpp
layout: document
redirect_from:
- /library/old/Lazy_Segment_Tree_Range_Maximum_Query.cpp
- /library/old/Lazy_Segment_Tree_Range_Maximum_Query.cpp.html
